...
df_prices = pd.read_csv(
    f"{data_path}/Filtrado/top40_cierre_aju.csv",
    parse_dates=['Date'],
    index_col='Date'
)
...
def performance_metrics(ds_port_returns: pd.Series, periodic_rate: int = 252) -> dict:
    ...
    if ds_port_returns.size == 0:
        return {
            "E(R)": 0.0,    # Retornos Anualizados
            "Std(R)": 0.0,  # Volatilidad Anualizada
...
df_bench = pd.read_csv(
    f'{data_path}/Crudo/splac_cierre_aju.csv',
    parse_dates=['Date'],
    index_col='Date'
)
...
fig.supxlabel('Fechas', x = 0.5, y=0, fontweight='bold')
