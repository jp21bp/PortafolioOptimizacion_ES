"""
Ingenieria de variable de Datos:

Indicadores tecnicos enfocados:
* Precios
...
Detalles del Modelo:
* Existen 3 diferentes modelos con la misma arquitectura:
    - Forma de los datos ingresantes = (t, k * n)
        * t = ventana del pasado = 50 dias
        * n = numero de activos = 21 activos
        * k = numero de indicadores tecnicos
* Indicadores tecnicos del Modelo 1:
    - Precios
    - Retorno Logaritmico
* Indicadores tecnicos del Modelo 2:
    - TEMA
    - HLC3
    - OBV
* Indicatores del Modelo 3:
    - Todos los 5 indicadores de arriba
"""
...
df_all = pd.read_csv(
    f"{data_path}/top40_completo.csv",
    parse_dates=['Date'],
    index_col='Date'
)
...
df_prices = pd.read_csv(
    f"{data_path}/top40_cierre_aju.csv",
    parse_dates=['Date'],
    index_col='Date'
)
