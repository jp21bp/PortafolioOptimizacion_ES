"""
Este archivo detalla la coleccion de datos utilizando el API de YFinance
"""
...
df_raw_AC.to_csv(f'{data_path}/Data/Crudo/top40_cierre_aju.csv', index=True, encoding='utf-8')

##### Saving all original multi level columns
df_raw.columns = ['_'.join(col).strip() for col in df_raw.columns.values]
df_raw.to_csv(f'{data_path}/Data/Crudo/top40_completo.csv', index=True, encoding='utf-8')
...
df_splac_AC.to_csv(f'{data_path}/Data/Raw/splac_cierre_aju.csv', index=True, encoding='utf-8', multi_level_index=False)
